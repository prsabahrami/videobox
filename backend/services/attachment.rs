...
use diesel::*;
use crate::services::attachment_blob::AttachmentBlobChangeset;
use crate::services::storage::{Storage, AttachmentBlob, Utc, ID};
...
impl Attachment {
    ...
    pub fn find_for_record(
        ...
        attachments::table
            .filter(attachments::name.eq(item_name))
            .filter(attachments::record_type.eq(item_record_type))
            .filter(attachments::record_id.eq(item_record_id))
            .first::<Self>(db)
    }
    ...
    pub fn find_all_for_records(
        ...
        attachments
            ...
            .filter(record_type.eq(item_record_type))
            .filter(record_id.eq_any(item_record_ids))
            .get_results::<Self>(db)
    }
